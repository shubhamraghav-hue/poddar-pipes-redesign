...
Crops of that second shot were saved under `build/tank-options/` with
...
| R4 | 770px | 68% | 67% | clips 33px of lid |

R4 shipped for a while, chosen deliberately over the fully-safe options: the
largest and furthest right, accepting a 33px lid clip while hovered as the
price of that. Those alternates have since been deleted — recover them from
git history if the single-tank framing is ever wanted back.

### LIVE — the tank pair, sized by script (Sep 2026)

Live asset: `public/products/category-cards/tank-card-g3-z660.png`, at
`photoPos: "50% 50%"`. This retired the whole R1-R4 exercise.

Generated by **`scripts/crop-tank-card.mjs`** — use it rather than cropping by
hand:

```
node scripts/crop-tank-card.mjs "<Tanks v3 (1).png>" 660 \n    --right 0.92 --floor 10 --slack 100 --tag g3
```

It auto-detects the tank bounding box (so it works on any future tank photo),
writes a `tank-card-z<width>.png` for each zoom step, and prints what each one
will actually do on the card. Two knobs only:

- **zoom** = crop WIDTH. Smaller = more zoomed = bigger tanks and less empty
  card.
- **`--right`** = where the tanks' right edge lands, 0-1. `0.92` is the
  current right-corner placement; `0.75` sits nearer the middle.
- **`--tag`** = a token folded into the output filenames. Pass it whenever the
  SOURCE photo changes (see the cache note below).

Vertical placement is *derived*, not a knob: the tanks are pushed down to sit
10px above the floor, because hover hides the TOP of the box and any headroom
above them is wasted space that pushes the lids into the hidden band.

Every output is cut to the photo box's own **1.5686:1**, so `object-cover` has
nothing left to trim and **`photoPos` is inert** — the crop rectangle is the
...
**reverted on request**: the frame is a single shared `h-[68%]` for all six
cards again, verified live — every frame measures identically
(`allFramesEqual: true`).

What replaced it is a cleaner split of responsibility:

| What | Where | Needs regeneration? |
| --- | --- | --- |
| Size (zoom) | script, bare number | yes |
| Horizontal framing | script, `--right` | yes |
| **Vertical placement** | **`photoPos` Y in the component** | **no** |

`photoPos` used to be **inert** on this card, because the crop was cut to
exactly the frame's aspect ratio and `object-cover` had no overflow to slide.
The script's new `--slack` cuts the crop 100px TALLER than the frame, which
restores that overflow — measured live: a 352x225 frame against an image
scaled to 352x278, so **53px of travel**, and the painted content provably
changes between Y=0% and Y=88%.

The crop bottom is anchored at `tank_y1 + floor + slack`, so the framing at
`Y = 0%` is identical to a slack-free crop. Slack only ever ADDS upward
travel; it never shifts the baseline.

`photoPos` X still does nothing — `object-cover` consumes the full width, so
there is never horizontal overflow. Sideways moves are `--right`.

**Uplift costs lid.** Hover hides a fixed 35.3% of the frame, so every pixel
of uplift pushes the lids further into that hidden band. At the live z660:

| photoPos Y | uplift | lid clipped on hover |
| --- | --- | --- |
| **0% (live)** | — | **37px** |
| 20% | +12px | 49px |
| 40% | +24px | 61px |
...
Past 88% the lids leave the frame even at rest. Note the constant frame costs
...
**35.3%**. Only 64.7% of the box survives, so **a tank taller than 64.7% of it
cannot survive hover intact, from any source, at any position.** Everything
below a 820px crop trades lid for size:

At the constant 68% frame, with `photoPos` Y at 0%:

| zoom | tank height | tank width | lid clipped on hover |
| --- | --- | --- | --- |
| z820 | 61.8% of frame | 39.5% | none |
| z800 | 63.3% | 40.5% | 1px |
| z780 | 65.0% | 41.5% | 6px |
| z750 | 67.6% | 43.2% | 13px |
| z720 | 70.4% | 45.0% | 20px |
| z700 | 72.4% | 46.3% | 25px |
| **z660 (LIVE)** | **76.7%** | **49.1%** | **37px** |
| z620 | 81.8% | 52.3% | 50px |

z660 is live deliberately: the brand team's reference framing has the tanks
filling most of the card, and the hover-clean options top out at 61.8% of the
frame, which reads noticeably small against the five pipe cards. **Do not
"fix" this back to z820 without asking** — it is a considered trade.

The ceiling is a *height* limit, which is why a pair works where one tall tank
could not: a squat pair buys presence in width instead.

Only the live crop is committed. The other zoom steps are ~700KB each, so
they are regenerated on demand rather than kept in the repo — the table above
is reproduced in `ProductCategories.tsx` so the trade-offs are readable
without running anything.

### Three traps this card set off, all worth knowing

**1. The optimiser cache cannot be cleared by deleting the cache directory.**
Next caches optimised images by source PATH. A re-shot arrived with a new
background gradient; the crops were regenerated under the SAME filenames and
`.next/cache/images` was deleted — and the browser still served the old
gradient. Proven by sampling the served pixels: `b-r` came back 44 (old)
while the file on disk measured 26 (new). **A new filename is the only
reliable fix**, which is what `--tag` exists for (`tank-card-g2-z660.png`).

**2. The tank detector was tuned to a backdrop that changed.** Tanks are
found by "bright AND neutral", because the navy backdrop is blue-dominant.
The first shot measured `b-r ~ +42`, so the light shaft failed that test. The
re-shot's gradient is much less blue (`b-r ~ +24`); the shaft went near-white,
passed, and the detected tank box came back **657px wide instead of 323px** —
which would have silently produced a completely different crop. Fixed with
`SKIP_TOP`: tanks stand on the floor, so the top 40% of the frame is excluded
outright. Verified to agree within 1px across both shots.

**3. A positional argument that was silently ignored.** `--right` used to be a
second positional, so `--all 0.80` parsed `0.80` as the *zoom* — which `--all`
then discards — leaving placement on its 0.92 default. The crops came out in a
different position than the command claimed, with nothing to indicate it. It
is now a named flag, and a bare number `<= 1` is rejected outright rather than
being quietly misread.

### Measuring hover in the browser needs a settle loop

Reading the transform straight after a synthetic hover returns a lift of 0 —
the CSS transition has not started — which reads as a false "no clipping"
pass. Poll until the lift equals 22.5% of the card width before trusting any
number. This bit once already.

If a re-shot is ever commissioned for a SINGLE tank, deliver at the card's own
photo-box aspect so nothing is cropped: **2400 × 1530** (1.5686:1), tank inside
a **641 × 900** box at left 1359 / top 580, centre 70% across, and nothing
...
glyph antialiasing only). It is **now live on `/about`** (Sep 2026), where
the same frame appears as Figma node 1001:5996. Its render in
`app/[locale]/page.tsx` is still commented out, so it is not on the homepage.

The About node and the landing node are the same 1512×1350 box with the same
artwork and the same six milestones at identical coordinates. Only two things
differ, so both are props rather than a forked component:
...
aspect-locked to 1512×1350, everything scaled with the viewport — on a 390px
phone the milestone copy measured **3.8px**. Invisible while the component was
unused; shipped the moment `/about` went live.

Below `md` the aspect-locked frame is now `hidden` and a stacked timeline
renders instead, built from the same `MILESTONES` array and the same colours —
a conventional fallback, not an invented redesign. Replace it with the real
thing when a mobile frame exists. Above `md` nothing changed.
...
## About Us page rebuilt from Figma (Sep 2026)

`/about` was rebuilt from scratch against Figma node 1001:5531. The previous
page and every component specific to it were deleted; everything the mock
already had a component for was reused rather than rewritten.

### Section map

| Figma node | Name | Implementation |
| --- | --- | --- |
| 1001:5975 | company overview | **new** `components/about/AboutHero.tsx` |
| 1032:8548 | Assurances | **new** `components/about/Assurances.tsx` |
| 1001:5996 | legacy section | **reused** `components/home/LegacyStory.tsx` |
| 1029:8208 | start a conversation | **reused** `CTASection variant="flush"` |
| 1029:8216 / 1001:5748 | footer / header | global layout, untouched |

The CTA needed no new copy at all: `home.ctaTitle`, `home.ctaDesc`,
`home.ctaPrimaryHome` and `home.ctaSecondaryHome` already carry node
1029:8208's exact strings, so the flush variant reproduces it as-is.

### What was deleted

Five components were About-only and went with the page: `OurStory.tsx`
(`AboutHero`, `OurStory`), `MissionVision.tsx` (`MissionVision`, `CoreValues`),
`Leadership.tsx`, `Timeline.tsx`, `Certifications.tsx`. `lib/data/team.ts` went
with `Leadership`, its only consumer.

`components/about/Facilities.tsx` **survives** — `/manufacturing` imports it.
Its second export `GlobalPresence` was About-only and was removed. Note the
trap here: a plain grep for "Certifications" and "Timeline" appears to show
other consumers, but those hits are a comment in `app/[locale]/page.tsx`, a
metadata title string in `quality/page.tsx`, and the unrelated
`components/home/QualityCertifications.tsx`. Only `Facilities` is genuinely
shared.

### Vision / Mission cards

The two panels (nodes 1029:8303 / 1029:8304) are a single vector whose
...
Figma exports the row dividers as two vectors ("L" / "R", nodes 1034:8592 /
1034:8593). Opened, each is nothing but five 0.5px `#606060` hairlines at
100px intervals — layout, not artwork — so they are CSS borders. Two defects
found by measuring rather than by eye, both since fixed:

- **Border outside the min-height box.** With `min-h-[100px]` on an inner div
  and `border-b` on the wrapping `<li>`, the two stack instead of nesting and
  every row became 100.8px. The last divider landed at 789 against Figma's
  784, drifting 1px per row. Both properties now sit on the same element, and
  the dividers land on 384 / 484 / 584 / 684 / 784 exactly.
- **Unequal columns.** `grid-flow-col` + `grid-rows-5` alone sizes each track
  to its content, and the 06–10 column (longest lines) came out 117px wider.
  `grid-cols-2` added; both columns are now 614px.

Column-major ordering is deliberate: `grid-flow-col` + `grid-rows-5` puts
01–05 left and 06–10 right per Figma, while a single `<ol>` still reads 01→10
in DOM order and collapses to one ordered column on mobile.

### Deliberate deviations from the node

- **Heading colour.** Figma draws "BACKED BY" `#606060` / "ASSURANCES"
  `#171796`. Rendered in the sitewide heading grey with a weight-only accent
  instead — the global heading spec was an explicit decision that outranks a
  per-node colour. Same call as in LegacyStory.
- **Left margin.** Figma's content band starts at x=133 (rules) / x=153
  (heading and numbers) in a 1512 frame. The site's `container-edge` puts them
  at 120 / 140 — a uniform 13px shift. Every internal relationship is exact
  (number→text gutter 24px, rule→number inset 42px), and the section now lines
  up with every other section's left edge rather than with one node's margins.
- **Section heights.** Cards are 622px wide against Figma's 592 (container-edge
  is wider than Figma's 1212 content band), so they are proportionally taller
...
A line drawing is not a photograph, and the card was built for a photograph.

- **It has to sit on a LIGHT card.** The linework measures `#2061a1`, which is
  **6.39:1 on white** but only **3.00:1** on the `rgba(9,12,40,…)` navy the
  photo treatment used. The card therefore gained an explicit `bg-white` —
  load-bearing, not decoration, because the artwork keeps its alpha and would
  otherwise show whatever the page background happens to be.
- **The scrim had to invert, and the caption with it.** The photo needed a
  near-black bottom gradient to carry white text. On paper that becomes a
  white gradient carrying dark text: caption `#0b0b52`, label `#171796`. The
  label specifically must NOT stay `amber-600` — the Playbook is explicit that
  orange text on a light surface fails AA (~2.7:1); it only ever worked here
  because it sat on the photo's dark scrim.
- **The caption had to move to the TOP.** This is the one worth remembering.
  The drawing's only empty region is its sky; the pipe run is along the bottom
  edge. A bottom scrim — the photo's placement — veiled precisely the part of
  the artwork worth showing on a piping site. Caption is now top-left with the
  gradient running downward, and the full drawing reads.

### Crop and aspect

The source is 1536×1024 and its left ~45% is nearly empty — measured column
density is flat until x≈768 (50%) and only dense from x≈960 (63%). Dropped
into a 331px box whole, the building would have been tiny and most of the card
blank. It is therefore pre-cropped to a **square, right-hand 1024×1024**
(`extract` at left 512), which keeps the building, the tree and the pipe
junction and discards the dead space.

Because the asset is already 1:1, the card is now `aspect-square` at **every**
width — where the photo was letterboxed to `aspect-[21/10]` while stacked. Any
other ratio crops either the building or the pipe run. Figma's literal square
from `md` up is unchanged.

Source was a transparent PNG (10.9% of pixels carry any alpha); as WebP,
251KB.
