...
Live asset: `public/products/category-cards/tank-card-g2-b72-z660.png`, at
...
```
node scripts/crop-tank-card.mjs "<Tanks v3 (1).png>" 660 --box 72 \n    --right 0.92 --floor 10 --tag g2-b72
```
...
### Per-card photo-box height (Sep 2026)

The photo box was `h-[68%]` in the shared JSX, so it could not be tuned for
one card. It is now `PHOTO_BOX_HEIGHT`, a per-id override map with 68% as the
default — **the TANKS card is at 72% and the five pipe cards are provably
untouched** (measured live: pipes 257px/68%, tanks 272px/72% on the same
378px card).

This is the cheapest win available on this card, because the hover lift is a
FIXED `22.5cqw`: a taller box is hidden proportionally LESS. At 68% the lift
hides 35.3% of the box; at 72% it hides 33.3%. That alone took the lid clip
from **37px down to 23px at the same tank size** — no re-zoom needed.

Two constraints on raising it further:

- The wordmark starts at `top-[74.67%]`. Figma leaves a 6.67% gap under the
  photo at 68%; at 72% that gap is 2.67%, and past ~74% the photo runs under
  the wordmark.
- **The box height sets the box's ASPECT RATIO**, and the crop is cut to match
  so `object-cover` has nothing to trim. Change `PHOTO_BOX_HEIGHT` and you
  must re-run the crop script with the same `--box` value, or the card
  silently crops the sides off the framing. This coupling is the sharp edge
  here.
...
At the current `--box 72` (the script prints this table for whatever `--box`
you give it):

| zoom | tank height | tank width | lid clipped on hover |
| --- | --- | --- | --- |
| z820 | 58.3% of box | 39.5% | none |
| z780 | 61.3% | 41.5% | none |
| z750 | 63.8% | 43.2% | none — largest that survives hover |
| z720 | 66.5% | 45.0% | 5px |
| z700 | 68.4% | 46.3% | 10px |
| **z660 (LIVE)** | **72.6%** | **49.1%** | **23px** |
| z620 | 77.3% | 52.3% | 35px |

z660 is live deliberately: the brand team's reference framing has the tanks
filling most of the card, and the hover-clean options top out at 63.8% of the
box, which reads noticeably small against the five pipe cards. 23px is under
the 33px clip R4 was knowingly shipped with. **Do not "fix" this back to z750
without asking** — it is a considered trade.
